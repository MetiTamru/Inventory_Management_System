...
const WeeklySale = () => {
  ...
  const [totalProfitWeekly, setTotalProfitWeekly] = useState(0);
  ...
  const displayData = () => {
    const WeeklyFiltered = salesData.filter((item) => isThisWeek(item.date));
    const totalProfitToday = WeeklyFiltered.reduce((acc, item) => acc + parseFloat(item.profit), 0);
    const totalSalesToday = WeeklyFiltered.reduce((acc, item) => acc + parseFloat(item.selling_price), 0);

    const totalQuantity = WeeklyFiltered.reduce((acc, item) => acc + parseFloat(item.quantity), 0);

    

    setTotalQuantity(totalQuantity)
    setTotalSalesWeekly(WeeklyFiltered.length); 
    setTotalSalesWeekly(totalSalesToday);
    setTotalProfitWeekly(totalProfitToday);
    setWeeklyData(WeeklyFiltered);
  };
  ...
  return (
    <div className="p-6 w-full bg-gray-100 h-screen mt-36">
      <div className="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6">
        <h1 className="text-3xl font-bold mb-6">Weekly Sales Report</h1>

        {/* Summary Section */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
          <div className="bg-gradient-to-b from-[#c684d0] via-[#eaceee] to-[#c684d0] p-4 rounded-lg shadow-md">
            <h2 className="text-lg font-semibold">Total Sales</h2>
            <p className="text-2xl font-bold text-green-700">${totalSalesWeekly.toFixed(2)}</p>
          </div>
          <div className="bg-gradient-to-b from-[#c684d0] via-[#eaceee] to-[#c684d0] p-4 rounded-lg shadow-md">
          ...
          <div className="bg-gradient-to-b from-[#c684d0] via-[#eaceee] to-[#c684d0] p-4 rounded-lg shadow-md">
            ...
            <p className="text-2xl font-bold text-red-700">${totalProfitWeekly.toFixed(2)}</p>
          </div>
        </div>
